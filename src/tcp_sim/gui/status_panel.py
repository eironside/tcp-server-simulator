...
class StatusPanel(ttk.LabelFrame):
	def __init__(self, master: tk.Misc) -> None:
		...
		ttk.Label(self, textvariable=self.connection_var).grid(row=0, column=0, sticky="w", padx=4, pady=2)
		ttk.Label(self, textvariable=self.progress_var).grid(row=1, column=0, sticky="w", padx=4, pady=2)
		ttk.Label(self, textvariable=self.rate_var).grid(row=2, column=0, sticky="w", padx=4, pady=2)
	...
	def update_rate(self, features_per_second: float, kb_per_second: float) -> None:
		self.rate_var.set(f"Rate: {features_per_second:.2f} feat/s | {kb_per_second:.2f} KB/s")
